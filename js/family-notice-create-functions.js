function renderFamilyNoticeCreate(detail) {
    atRestInitCheckboxes();
    atRestInitDelete();
}

window.addEventListener('postsRendered', renderFamilyNoticeCreate);
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (urlParams.get('saved') === '1') {
        showGlobalAlert('Family notice successfully added');
    }

    if (urlParams.get('draft') === '1') {
        showGlobalAlert('Family notice successfully saved as draft');
    }

    if (localStorage.getItem('death_notice_deleted') === '1') {
        showGlobalAlert('Family notice(s) successfully deleted');
